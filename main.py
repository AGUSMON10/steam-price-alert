...
skins_a_vigilar = {
    ...
    "★ StatTrak™ Falchion Knife | Damascus Steel (Minimal Wear)": 145.00,
    ...
    "StatTrak™ USP-S | Neo-Noir (Minimal Wear)": 130.00,
    "StatTrak™ USP-S | Printstream (Well-Worn)": 63.00,
    "StatTrak™ Desert Eagle | Ocean Drive (Minimal Wear)": 145.00,
    ...
    "★ StatTrak™ Bowie Knife | Marble Fade (Factory New)": 205.00,
...
def dashboard():
    ...
    if estado_app["ultimo_escaneo"]:
        ultimo_escaneo = estado_app["ultimo_escaneo"]
    else:
        ultimo_escaneo = "Todavía no hay datos"
                ...
                <tbody>

                    {
                        "".join(
                            ...
                            <tr>
                                <td>{skin}</td>
                                <td>${datos.get("pagado", 0):.2f}</td>
                                <td>${datos.get("perdida_maxima", 0):.2f}</td>
                            </tr>
                            '''
                            for skin, datos in skins_a_vigilar.items()
                        )
                    }
